...
#pragma warning disable IDE1006 // Naming Styles
    public async Task<CFContentViewModel> SendHttpRequestAsyncNew<TResponse>(GraphQLRequest request, string env, string collection, string tpid, string fundName, CancellationToken cancellationToken = default)
#pragma warning restore IDE1006 // Naming Styles
    {
        tpid = tpid.ToUpper();
        var viewModel = new CFContentViewModel();
        try
        {
            ...
            var result1 = JsonConvert.DeserializeObject<dynamic>(str1);
            var jObject = JObject.Parse(str1);
            var list = jObject
                .Descendants()
                .OfType<JObject>()
                .Where(jp => jp.Children<JProperty>().Any(token => token.Name == "items"))
                .Descendants()
            ...
            foreach (var item in list)
            {
                ...
                foreach (var s in item)
                {
                    ...
                    var model = new ContentDisplayModelItem
                    {
                        ...
                        FieldName = values[0].Replace("[", ""),
                        FieldValue = values[1].Replace("]", ""),
                        FundName = fundName
                    };
    ...
    private async Task<GraphQLHttpResponse<TResponse>> SendHttpRequestAsync<TResponse>(GraphQLRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var len = httpResponseMessage.Content.Headers.ContentLength.Value;


        contentStream.Read(sevenThousandItems, 0, (int)len).ToString();
        var str = System.Text.Encoding.Default.GetString(sevenThousandItems);
...
public class CFContentViewModel
{
